...
object SetUp {
  ...
  def generate_pehash(id: String) : Array[Byte] = {
    ...
    val query = sc.cassandraTable(keyspace, knowledge_base_table).where("object_id = ?",id).map(x=> (x.get[String]("feature_type"),x.get[Array[Byte]]("feature_value"),x.get[java.util.UUID]("timestamp"))).filter(x=> x._1==feature_type).sortBy(x=> x._3,false).cache()

    if (!query.isEmpty()) {
      val value = query.map(x=> decompress(x._2)).first
      val search = sc.cassandraTable(keyspace, mv_feature_type_table).where("feature_type =?",feature_type).map(x=> (x.get[String]("object_id"), decompress(x.get[Array[Byte]]("feature_value")), x.get[java.util.UUID]("timestamp"))).filter(x=> x._1 != id).sortBy(x=> (x._1, x._3),false)
