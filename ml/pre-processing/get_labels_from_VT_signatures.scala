...
val VT_sample_signatures_final_array_rdd = spark.read.format("parquet").load("./VT_sample_signatures_final_array.parquet").rdd.map(row => new VT_sample_signatures_final_array_rdd_class(row(0).toString,row(1).asInstanceOf[Seq[Double]].toArray))
...
VT_sample_label_rdd.toDF().write.format("parquet").save("./VT_sample_label.parquet")
