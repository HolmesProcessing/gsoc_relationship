...
val objdump_main_list = sc.textFile("./t0day/x86Opcodes").collect.toList
...
val objdump_results_by_service_name_meta = sc.cassandraTable[objdump_results_by_service_name_class]("gsoc3","results_meta_by_service_name").where("service_name=?","objdump")
val objdump_results_by_service_name_rdd = objdump_results_by_service_name_meta.keyBy(x=> (x.sha256,x.service_name))
val objdump_results_by_sha256_meta = sc.cassandraTable[objdump_results_by_sha256_class]("gsoc3","results_data_by_sha256")
...
objdump_binaray_final_array_rdd.toDF().write.format("parquet").save("./objdump_binaray_final_array.parquet")
