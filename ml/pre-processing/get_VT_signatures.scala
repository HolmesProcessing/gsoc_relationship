...
val signatures_prefix_rdd = sc.textFile("./prefix.txt").map(x=>x.toLowerCase())
...
VT_sample_signatures_final_array_rdd.toDF().write.format("parquet").save("./VT_sample_signatures_final_array.parquet")
