...
val VT_results_by_service_name_meta = sc.cassandraTable[VT_results_by_service_name_class]("gsoc3","results_meta_by_service_name").where("service_name=?","virustotal")
val VT_results_by_service_name_rdd = VT_results_by_service_name_meta.keyBy(x=> (x.sha256,x.service_name))
val VT_results_by_sha256_meta = sc.cassandraTable[VT_results_by_sha256_class]("gsoc3","results_data_by_sha256")
...
val signatures_prefix_rdd = sc.textFile("./t0day/prefix.txt").map(x=>x.toLowerCase())
...
VT_sample_signatures_final_array_rdd.toDF().write.format("parquet").save("./t0day/VT_sample_signatures_final_array.parquet")
